"""
HNTV v8 Training — DDP via Accelerate
v8: v6.5 레시피에 LoRA r=64->128, alpha=128->256(비율 유지)만 바꿈. 원래 v6.5는 hard negative를 학습 시작 전
한 번만 고정하지 않고, 매 __getitem__ 호출마다(=매 epoch마다 다른 조합으로) 새로 샘플링.
...
def kendall_tau_distance(p, q):
    """두 순열(값 1..4의 배치) 사이의 Kendall tau distance (뒤바뀐 쌍의 개수, 0~6)."""
    pos_p = {v: i for i, v in enumerate(p)}
    pos_q = {v: i for i, v in enumerate(q)}
    d = 0
    for x in range(1, len(p) + 1):
        for y in range(x + 1, len(p) + 1):
            if (pos_p[x] < pos_p[y]) != (pos_q[x] < pos_q[y]):
                d += 1
    return d
...
def val_exact_match(model, processor, val_raw_df, yes_id, no_id, device, n_samples=None):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="val", leave=False):
        ...
        if best == gt:
            ...
            kd = kendall_tau_distance(best, gt)
            wrong_cases.append({"id": sample_id, "gt": gt, "pred": best, "type": t,
                                 "kendall_d": kd, "no_ordering": is_no_ord})

    acc = correct / len(df)
    adj = sum(1 for w in wrong_cases if w["type"] == "adj_swap")
    kd_hist = {d: sum(1 for w in wrong_cases if w["kendall_d"] == d) for d in range(1, 7)}
    print(f"\n[Val] exact_match={acc:.4f} ({correct}/{len(df)})  "
          f"adj_swap_fail={adj}  other_fail={len(wrong_cases) - adj}")
    print(f"  실패 사례의 Kendall distance 분포(예측이 정답과 얼마나 다른가): "
          + ", ".join(f"d{d}={kd_hist[d]}" for d in range(1, 7)))
    if no_ord_total > 0:
        print(f"  No_ordering 샘플([1,2,3,4] 고정 정답) 별도 정확도: "
              f"{no_ord_correct}/{no_ord_total} = {no_ord_correct/no_ord_total:.4f}")
    model.train()
    return acc
...
def main():
    ...
    parser.add_argument("--vision_lr",  type=float, default=VISION_LR,
                         help="vision encoder LoRA 전용 LR (LLM LoRA와 분리, 보통 더 낮게)")
    parser.add_argument("--ckpt_name",  type=str,   default="best_v12")
    parser.add_argument("--resume_from", type=str,  default=None,
                         help="job이 중간에 끊겼을 때 대비: 저장된 LoRA 체크포인트 경로(예: checkpoints/best_v12_last)"
    ...
    if is_main:
        ...
        print(f"[v12] Processes={accelerator.num_processes}  Device={device}")
        print(f"  lora_r={args.lora_r}  lora_alpha={args.lora_alpha}  lr={args.lr}  vision_lr={args.vision_lr}")
        print(f"  Loss: AdaptiveDistanceLoss (temp={WEIGHT_TEMP}, all d=1..6 covered, "
              f"No_ordering도 동일 hard-negative 처리)")
    ...
    # ── 데이터 ─────────────────────────────────────────────────────
    # v8(v6.5 상속): hard negative를 학습 전 CSV로 고정하지 않고, GroupedTemporalDataset이
    ...
    # vision LoRA는 LLM LoRA와 분리된 LR(보통 더 낮게)을 쓰도록 param group 분리.
    # get_cosine_schedule_with_warmup은 각 group의 base lr을 독립적으로 스케일링하므로
    # 두 그룹의 LR 비율(lr/vision_lr)은 학습 내내 그대로 유지됨.
    llm_params, vision_params = [], []
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        (vision_params if "visual." in n else llm_params).append(p)
    if is_main:
        print(f"  optimizer param groups: llm={len(llm_params)}개(lr={args.lr}), "
              f"vision={len(vision_params)}개(lr={args.vision_lr})")
    optimizer = AdamW([
    ...
    for epoch in range(args.start_epoch, EPOCHS + 1):
        ...
        model.train()
        epoch_loss = 0.0
        # 에폭 전체에 걸친 거리별(d=1..6) 평균 loss 및, 최근 로깅 구간의 no_ordering 그룹 비중
        # (step별 loss spike가 no_ordering 샘플 쏠림 때문인지 확인하기 위함)
        epoch_dist_sum   = {d: 0.0 for d in range(1, 7)}
        epoch_dist_cnt   = {d: 0   for d in range(1, 7)}
        recent_no_ord    = 0
        recent_total_grp = 0
        ...
        for step, batch in enumerate(tqdm(train_dl, desc=f"Ep{epoch}", disable=not is_main)):
            with accelerator.accumulate(model):
                ...
                texts, imgs_list = [], []
                group_offsets   = []  # (start, size, grp_dists)
                ...
                recent_total_grp += len(batch["no_ordering"])
                recent_no_ord    += sum(1 for x in batch["no_ordering"] if x)
                ...
                for d, l in per_dist_loss.items():
                    epoch_dist_sum[d] += l
                    epoch_dist_cnt[d] += 1

                if is_main and global_step % LOGGING_STEPS == 0:
                    lr_now  = scheduler.get_last_lr()[0]  # llm 그룹 lr (group 0)
                    vlr_now = scheduler.get_last_lr()[1]  # vision 그룹 lr (group 1)
                    avg    = epoch_loss / (step + 1)
                    w = weights.tolist()
                    e = criterion.ema.tolist()
                    L = [per_dist_loss.get(d, float("nan")) for d in range(1, 7)]
                    max_d = max(per_dist_loss, key=per_dist_loss.get) if per_dist_loss else -1
                    no_ord_ratio = recent_no_ord / max(recent_total_grp, 1)
                    print(
                        f"  step={global_step:5d}  loss={avg:.4f}  lr={lr_now:.2e}/{vlr_now:.2e}"
                        f"  w=[{','.join(f'{x:.3f}' for x in w)}]"
                        f"  L=[{','.join(f'{x:.3f}' if not (x!=x) else 'nan' for x in L)}]"
                        f"  max_loss_d={max_d}"
                        f"  no_ordering비중(최근구간)={no_ord_ratio:.2f}({recent_no_ord}/{recent_total_grp})"
                    )
                    recent_no_ord, recent_total_grp = 0, 0
        ...
        if is_main:
            ...
            dist_avg_str = ", ".join(
                f"d{d}={epoch_dist_sum[d]/epoch_dist_cnt[d]:.4f}" if epoch_dist_cnt[d] > 0 else f"d{d}=nan"
                for d in range(1, 7)
            )
            print(f"  거리별 평균 loss(에폭 전체): {dist_avg_str}")
