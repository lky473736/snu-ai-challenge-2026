...
### TPRU fine-tuning 진행 상황 (Job 225744, 2026-07-02)

변경 사항 (이전 runs 대비):
- **No_ordering 샘플 포함**: hard_negative.py의 skip 제거 → 7658 → 9059 groups (+18.3%)
  - No_ordering=True인 1401개 샘플도 positive + 3 adjacent swap hard neg으로 학습
- **데이터셋**: positive 9059 + hard_neg 27177 = 36236 총 샘플
- MAX_IMAGE_SIZE: 560 시도 → 16s/step으로 너무 느려서 448로 복구 (11.5s/step)

속도: 11.5s/step, 2264 steps/epoch → **7.25h/epoch**, 24h 안에 **3-3.5 epoch** 커버

---

## 12. 실험 4 (Video-R1-7B): T-GRPO Temporal Contrastive Base (2026-07-02)

### 배경

**Video-R1** (arXiv:2503.21776, 2025-03-27 공개)
- Base: Qwen2.5-VL-7B-Instruct (TPRU와 동일 계열)
- 학습: **T-GRPO** (Temporal Group Relative Policy Optimization)
  - 같은 영상에 대해 (1) 정답 순서 프레임 (2) 랜덤 섞인 프레임 두 그룹 생성
  - 정답/오답 구분 능력이 클수록 높은 보상 → 우리 margin ranking loss와 **동일한 원리**
- 학습 도메인: 일반 영상 혼합 (TPRU보다 훨씬 넓음 — 사람 행동 포함) → 대회 도메인에 더 가까울 가능성
- 라이선스: Apache-2.0 ✓, 공개일 2025-03-27 (대회 기준 2026-05-31 이전) ✓
- HF: `Video-R1/Video-R1-7B`
- 벤치마크: VSI-Bench 37.1% (GPT-4o 초과), TempCompass·MVBench 개선

### TPRU vs Video-R1 비교

| | TPRU-7B | Video-R1-7B |
|---|---|---|
| Base | Qwen2.5-VL-7B | Qwen2.5-VL-7B |
| 학습 방식 | GRPO (frame reordering 직접 출력) | T-GRPO (ordered vs shuffled contrastive) |
| 학습 도메인 | 로봇/레고/GUI (좁음) | 일반 영상 혼합 (넓음) |
| 우리 태스크 유사도 | 높음 (같은 태스크 포맷) | 매우 높음 (같은 보상 원리 + 더 넓은 도메인) |
| Zero-shot | 26.3% | 미확인 (zero-shot eval 예정) |

### 구현

- 폴더: `/data/gyuyeonlim/hntv_VideoR1/` (hntv_TPRU 복사 + MODEL_PATH만 변경)
- 모델 다운로드: PID 1328502, `/data/gyuyeonlim/models/VideoR1-7B`
- `run_eval_zero_shot.sh`: zero-shot val 평가 (225744 완료 후 제출)
- `run_train_videor1.sh`: LoRA 학습 (24h, 4×H100, 동일 하이퍼파라미터)

### 실험 순서

1. 다운로드 완료 대기
2. 225744(TPRU) 끝나면 → zero-shot eval 제출
3. Video-R1 zero-shot vs TPRU zero-shot(26.3%) 비교
4. 더 높거나 비슷하면 fine-tuning 진행

---

## 13. Inference 개선 전략 (2026-07-02)

### TTA (Test Time Augmentation) — v1 기준 시도

구현: `/data/gyuyeonlim/hntv/hntv/src/inference_tta.py`
- 24순열 × (원본 + 좌우반전) 평균 → argmax
- 프롬프트 개선: "object positions, body postures, state changes" 명시 추가
- 체크포인트: `best_v1_score0.79057`
- 예상 소요: ~130분 (기존 65분 × 2)
- 코랩 A100용 zip: `/data/gyuyeonlim/hntv_tta.zip` (2.6GB)

효과 추정: 0~+1% (VLM에서 TTA 효과는 CNN보다 작음, 방향 정보가 중요한 장면에서는 노이즈 가능)

### Quantization 전략

규칙: "Quantization은 허용함" 명시 ✓

| | bfloat16 | INT8 | INT4 (GPTQ/AWQ) |
|---|---|---|---|
| 속도 | 기준 | ~1.5-2x 빠름 | ~3-4x 빠름 |
| 메모리 | ~14GB | ~7GB | ~4GB |
| 성능 하락 | 없음 | <0.5% | ~1-3% |

**적용 시점**: H100(80GB) 학습은 bfloat16 그대로. **RTX 3090(24GB) 최종 제출**에서만 INT4 고려.
- bfloat16 7B + activation + 4이미지 = 24GB에서 빡빡
- INT4: 4GB 모델 → 여유, 추론 더 빠름
