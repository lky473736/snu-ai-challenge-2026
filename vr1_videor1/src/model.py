"""
TPRU-7B (Qwen2.5-VL 기반) + LoRA
TPRU: RL로 temporal reordering 사전학습된 모델 → 우리 도메인 LoRA로 domain 적응
...
def _get_model_class(model_path: str):
    ...
    if _HAS_QWEN25 and getattr(cfg, "model_type", "") == "qwen2_5_vl":
...
def load_model_and_processor(model_path: str = MODEL_PATH, lora_r: int = LORA_R, lora_alpha: int = LORA_ALPHA):
    ...
    print(f"Loading model ({ModelClass.__name__}, bfloat16)...")
