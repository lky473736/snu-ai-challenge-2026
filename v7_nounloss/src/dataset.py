...
from src.loss import noun_bucket
...
class GroupedTemporalDataset(Dataset):
    ...
    v7: df에 n_nouns 컬럼이 미리 병합되어 있어야 함 (precompute_n_nouns.py로 1회 계산) —
    그룹(=문장 1개)이 속한 n_noun 버킷을 AdaptiveDistanceNounLoss에 넘기기 위함.
    df는 원본 1행 = 1샘플(Id, Sentence, Answer, n_nouns)."""
    ...
    def __getitem__(self, idx):
        ...
        nb  = noun_bucket(int(row["n_nouns"]))
...
def collate_fn(batch):
    return {
        ...
        "noun_buckets": [b["noun_bucket"] for b in batch],
    }
