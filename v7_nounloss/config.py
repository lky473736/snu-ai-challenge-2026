...
# 아직 이 축의 실측 dist_weights.csv 데이터가 없어(첫 도입) distance축과 동일한 temp=6.0에서
# 시작 — 학습 로그(wn 컬럼)로 실제 분화 정도를 확인해 필요시 조정할 것 (WEIGHT_TEMP 때와 동일한
# "2.0은 너무 약했다"는 교훈 반복 방지를 위해 처음부터 검증된 스케일로 시작).
NOUN_WEIGHT_TEMP   = 6.0
