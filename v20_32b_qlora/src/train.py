"""
HNTV v20 Training — Qwen3-VL-32B-Instruct QLoRA(4bit), DDP via Accelerate
v14 레시피(LoRA r=128/alpha=256, ListwiseSoftmaxLoss, hard_negative K=7 d=1..6 전 구간) 100% 유지,
베이스 모델만 8B bf16 -> 32B QLoRA(4bit)로 교체한 단일변수 실험. EPOCHS=3(탐색 실행).
...
# 스모크를 따로 안 돌리는 대신, 실제 학습/검증 스텝 안에서 OOM이 나면 chunk_size를 줄이고
# 그 값을 "계속" 쓴다(리스트로 감싸서 함수 밖에서도 갱신되게) — 매번 원래 값부터 재시도하는
# 낭비를 피함. 즉 이 잡의 초반 몇 스텝이 사실상 스모크 역할을 겸함(같은 프로세스, 실제 데이터로).
# 학습용/검증용(24-permutation, 배치가 훨씬 큼) 각각 독립적으로 축소·기억한다.
...
def main():
    ...
    parser.add_argument("--ckpt_name", type=str, default="best_v20")
    ...
    if is_main:
        ...
        print(f"[v20] Processes={accelerator.num_processes}  Device={device}")
        print(f"  base=Qwen3-VL-32B-Instruct QLoRA(4bit-NF4)  lora_r={args.lora_r}  lora_alpha={args.lora_alpha}  lr={args.lr}")
        print(f"  Loss: ListwiseSoftmaxLoss (v14와 동일, 8-way joint softmax)")
        print(f"  Hard negatives: LIVE resampled every __getitem__ (v14와 동일, K=7 d=1..6)")
        print(f"  EPOCHS={EPOCHS} (탐색 실행)")
        ...
        for step, batch in enumerate(tqdm(train_dl, desc=f"Ep{epoch}", disable=not is_main)):
            with accelerator.accumulate(model):
                ...
                try:
                    ...
                    # forward chunk_size와 별개로, group 전체(size=8)의 backward 그래프 자체가
                    # 너무 클 수 있음 — chunk_size를 더 줄여봤자 안 먹힐 수 있는 지점이라, 이 스텝은
                    # 건너뛰고(잡 안 죽임) 계속 반복되면 config의 SAMPLE_COUNTS(그룹 크기) 축소가
                    # 필요하다는 신호로 남긴다.
                    optimizer.zero_grad()
                    torch.cuda.empty_cache()
                    if _current_minibatch[0] > 1:
                        _current_minibatch[0] = max(1, _current_minibatch[0] // 2)
                    print(f"[OOM-backward] step {global_step} 스킵, TRAIN_MINIBATCH->{_current_minibatch[0]}. "
                          f"계속 반복되면 config.SAMPLE_COUNTS(그룹 크기=8) 축소가 필요할 수 있음.", flush=True)
                    continue
