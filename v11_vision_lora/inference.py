"""
v8 Inference (LoRA r=128) — DDP, 24-permutation exhaustive search (Qwen3-VL-8B base, 448px)
best_v11 and best_v11_last checkpoints → submission_v11_best.csv / submission_v11_last.csv
"""
...
CKPT_BASE  = Path("/data/gyuyeonlim/snu_ai_challenge/v11_vision_lora/checkpoints")
DATA_DIR   = Path("/data/gyuyeonlim/snu_ai_challenge/data/snuaichallenge_data")
OUT_DIR    = Path("/data/gyuyeonlim/snu_ai_challenge/v11_vision_lora")
V8_CKPT    = Path("/data/gyuyeonlim/snu_ai_challenge/v8_lora128/checkpoints/best_v8")

CKPT_LIST = [
    ("best_v11",      "submission_v11_best"),
    ("best_v11_last", "submission_v11_last"),
]
...
def main():
    ...
    # v11 = freeze 전략: v11 체크포인트는 vision-only LoRA이므로, 그 아래 LLM은
    # 반드시 v8의 LoRA가 병합된 상태여야 함 (train 때와 동일 조건).
    if rank == 0:
        print(f"[freeze] Merging v8 LoRA from {V8_CKPT} into base before loading v11 vision LoRA")
    base_model = PeftModel.from_pretrained(base_model, str(V8_CKPT), is_trainable=False)
