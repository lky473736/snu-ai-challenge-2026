...
class GroupedTemporalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # 이미지 4장 한 번만 로드 → 각 permutation에서 재배열만
        try:
            base_imgs = [load_image(str(img_dir / f)) for f in input_files]
        except Exception as e:
            print(f"[dataset] 이미지 로드 실패 {sample_id}: {e}")
            base_imgs = [Image.new("RGB", (224, 224)) for _ in range(4)]
        ...
        for _, row in group_df.iterrows():
            ...
            images = [base_imgs[inverse[t]] for t in range(4)]  # 재배열만
