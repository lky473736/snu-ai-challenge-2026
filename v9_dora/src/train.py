"""
HNTV v9 Training — DDP via Accelerate
v9: v8(r=128/alpha=256/LR=5e-5) 레시피에 PiSSA(init) + rsLoRA(scaling) + LoRA+(옵티마이저 A/B
LR 분리) 추가. DoRA는 rank 무관 고정 VRAM 오버헤드로 OOM 확인되어 폐기. 원래 v6.5는 hard negative를 학습 시작 전
...
from config import (
    ...
    EMA_ALPHA, WEIGHT_TEMP, LORA_PLUS_RATIO,
)
...
def main():
    ...
    parser.add_argument("--ckpt_name",  type=str,   default="best_v9")
    parser.add_argument("--resume_from", type=str,  default=None,
                         help="job이 중간에 끊겼을 때 대비: 저장된 LoRA 체크포인트 경로(예: checkpoints/best_v9_last)"
    ...
    if is_main:
        ...
        print(f"[v9] Processes={accelerator.num_processes}  Device={device}")
    ...
    # ── 데이터 ─────────────────────────────────────────────────────
    # v9(v8 상속): hard negative를 학습 전 CSV로 고정하지 않고, GroupedTemporalDataset이
    ...
    # LoRA+: lora_B는 0 초기화라 초반 그래디언트가 약함 -> B에 A보다 LORA_PLUS_RATIO배 높은 LR.
    # 파라미터 이름으로 A/B를 나누고(사전 확인: 이 모델에서 학습 파라미터는 전부 lora_A/lora_B
    # 둘 중 하나에 속함, 그 외 0개), 두 그룹을 별도 param_group으로 옵티마이저에 전달.
    # get_cosine_schedule_with_warmup은 param_group마다 자신의 초기 LR을 기준으로 동일한
    # 배율(warmup/decay factor)을 곱하는 방식이라 두 그룹 모두 자동으로 정상 스케줄링됨.
    ...
    assert len(other_params) == 0, f"lora_A/B 외 학습 파라미터 {len(other_params)}개 발견 — LoRA+ 그룹 분리 가정이 깨짐"
    if is_main:
        print(f"[LoRA+] lora_A params: {len(lora_a_params)}  lora_B params: {len(lora_b_params)}  "
              f"lr_A={args.lr:.2e}  lr_B={args.lr * LORA_PLUS_RATIO:.2e} (ratio={LORA_PLUS_RATIO})")
    optimizer = AdamW([
        {"params": lora_a_params, "lr": args.lr},
        {"params": lora_b_params, "lr": args.lr * LORA_PLUS_RATIO},
    ], weight_decay=0.01)
