...
from config import MODEL_PATH, LORA_R, LORA_ALPHA, LORA_DROPOUT, LORA_INIT, USE_RSLORA
...
def load_model_and_processor(model_path: str = MODEL_PATH, lora_r: int = LORA_R, lora_alpha: int = LORA_ALPHA,
    ...
    if resume_from:
        ...
        # v9: DoRA는 r=128~64 전부 OOM(rank 무관 고정 오버헤드)이라 포기.
        # PiSSA(사전학습 가중치를 SVD 분해해 상위 r개 주성분으로 A/B 초기화) + rsLoRA(scaling을
        # alpha/r 대신 alpha/sqrt(r)로, 고랭크에서도 안정적으로 큰 scale 사용 가능) 조합.
        # 세 기법(PiSSA/rsLoRA/이후 추가할 LoRA+) 모두 forward/backward 구조는 vanilla LoRA와
        # 동일해서 DoRA와 달리 메모리 오버헤드 없음(PiSSA는 초기화 시 1회성 SVD 계산만 추가).
