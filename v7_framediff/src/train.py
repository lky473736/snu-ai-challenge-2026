...
frame-diff 이미지(src/frame_diff.py, 인접 프레임 pixel diff 3장)를 추가 입력으로 넣어
d=1(인접 스왑) 오류를 직접 겨냥. EDA에서 해상도 상향은 이 문제를 못 줄인다는 게 확인되어
("어디를 봐야 할지"의 문제) 명시적으로 diff를 하이라이트해주는 방향으로 접근.
...
def val_exact_match(model, processor, val_raw_df, yes_id, no_id, device, n_samples=None):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="val", leave=False):
        ...
        for perm in ALL_PERMS:
            ...
            msgs, full_imgs = build_messages(imgs, sentence)
            text = processor.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
            all_texts.append(text)
            all_imgs.append(full_imgs)
        with torch.no_grad():
            ...
            with accelerator.accumulate(model):
                ...
                for grp_imgs, grp_sents, grp_dists in zip(
                    ...
                    for imgs, sent in zip(grp_imgs, grp_sents):
                        msg, full_imgs = build_messages(imgs, sent)
