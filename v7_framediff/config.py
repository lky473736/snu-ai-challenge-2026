...
# ── 학습 ───────────────────────────────────────────────────
LR            = 1e-5
# v7: v6.5(live resampling, EPOCHS=10)가 best val=0.6029(epoch5)로 최고 기록.
# 그 위에 frame-diff 이미지(src/frame_diff.py)를 추가해 d=1(인접 스왑) 문제를 직접 겨냥.
# EPOCHS=10 유지 (v6.5에서 과적합 지연 효과 확인됨).
EPOCHS        = 10
BATCH_SIZE    = 1
GRAD_ACCUM    = 8
WARMUP_RATIO  = 0.05
...
# ── Inference ──────────────────────────────────────────────
INFER_BATCH_SIZE = 24
# v7: 후보당 이미지 4장->7장(diff 3장 추가, 112px라 토큰 비용은 작음)으로 늘어서 VRAM 재확인 필요.
# _forward_chunk의 OOM 자동 축소 안전장치가 있어 8로 시작, 스모크테스트로 실제 안전 여부 확인.
TRAIN_MINIBATCH  = 8
