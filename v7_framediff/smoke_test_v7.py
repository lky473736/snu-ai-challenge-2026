...
# 실제 train.py는 BATCH_SIZE=1이라 매 스텝 그룹 1개(1 pos+7 neg=8샘플)만 forward+즉시backward함.
# 여러 그룹을 forward만 몰아서 하고 backward를 나중에 한 번에 하면 activation이 안 풀려서
# 실제보다 메모리를 더 많이 씀(v6 스모크 때 겪은 OOM 원인) -> 그룹 1개로 정확히 재현.
...
for grp_imgs, grp_sents, grp_dists in zip(batch["images"], batch["sentences"], batch["dists"]):
    ...
    for imgs, sent in zip(grp_imgs, grp_sents):
        ...
        texts.append(text)
        imgs_list.append(full_imgs)  # 4 원본 + 3 diff = 7장/후보
    group_offsets.append((start, len(grp_imgs), grp_dists))

print(f"후보당 이미지 수: {len(imgs_list[0])} (원본 4 + diff 3 기대)")
