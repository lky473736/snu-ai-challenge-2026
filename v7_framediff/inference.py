"""
v7 Inference — DDP, 24-permutation exhaustive search (Qwen3-VL-8B base, 448px + frame-diff 이미지)
...
from src.frame_diff import compute_diff_images

MODEL_PATH = "/data/gyuyeonlim/models/Qwen3-VL-8B-Instruct"
CKPT_BASE  = Path("/data/gyuyeonlim/snu_ai_challenge/v7_framediff/checkpoints")
DATA_DIR   = Path("/data/gyuyeonlim/snu_ai_challenge/data/snuaichallenge_data")
OUT_DIR    = Path("/data/gyuyeonlim/snu_ai_challenge/v7_framediff")
...
PROMPT_4F = (
    ...
    "The next 3 images show the pixel differences between consecutive frames "
    "(Diff 1-2, Diff 2-3, Diff 3-4) — bright regions mark where things changed.\n"
    "Please carefully examine the changes between consecutive frames, using the diff "
    "images to spot subtle differences that might otherwise be missed.\n"
...
SYSTEM = (
    ...
    "Given video frames in a specific order, their pairwise difference images, and a caption, "
...
def build_msgs(images, sentence):
    """반환: (messages, all_images) — dataset.py의 build_messages와 동일 계약 (학습/추론 일치 필수)."""
    diffs = compute_diff_images(images)
    all_images = list(images) + diffs

    content = []
    for i, img in enumerate(images, 1):
        content.append({"type": "text",  "text": f"Frame {i}:"})
        content.append({"type": "image", "image": img})
    for i, dimg in enumerate(diffs, 1):
        content.append({"type": "text",  "text": f"Diff {i}-{i+1}:"})
        content.append({"type": "image", "image": dimg})
...
def score_24perms(model, processor, yes_id, no_id, device, base_imgs, sentence):
    ...
    for perm in ALL_PERMS:
        ...
        msg, full_imgs = build_msgs(imgs, sentence)
        text = processor.apply_chat_template(msg, tokenize=False, add_generation_prompt=True)
        texts.append(text)
        imgs_list.append(full_imgs)
    with torch.no_grad():
