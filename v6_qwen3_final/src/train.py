"""
HNTV v6 Training — DDP via Accelerate
v6: Qwen3-VL-8B base + 512px + WEIGHT_TEMP=6 + No_ordering을 Ordered와 동일하게 hard-negative 학습
...
def main():
    ...
    parser.add_argument("--ckpt_name",  type=str,   default="best_v6")
    parser.add_argument("--resume_from", type=str,  default=None,
                         help="job이 중간에 끊겼을 때 대비: 저장된 LoRA 체크포인트 경로(예: checkpoints/best_v6_last)"
    ...
    if is_main:
        ...
        print(f"[v6] Processes={accelerator.num_processes}  Device={device}")
    ...
    if is_main:
        ...
        df_train.to_csv(CKPT_DIR / "_hn_v6.csv", index=False)
    ...
    df_train = pd.read_csv(CKPT_DIR / "_hn_v6.csv")
