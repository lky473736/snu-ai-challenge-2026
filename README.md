...
> - v1-v19, v21은 이 최종 모델에 도달하기까지의 실험 히스토리이며, 최종 제출 모델이 아닙니다(§7 표 참고).
...
6. [코드 및 데이터 사용 관련 준수 사항](#6-코드-및-데이터-사용-관련-준수-사항)
7. [프로젝트 구조 (버전 히스토리 v1-v21)](#7-프로젝트-구조-버전-히스토리-v1-v21)
8. [라이선스](#8-라이선스)
...
**버전별(v1-v21) 리더보드 점수 추이** — 실제 제출된 버전들의 public LB 점수(§7 표와 동일 데이터).
...
정확한 서버 사양과 CUDA 버전 주의사항은 §1을 참고하세요 — 반드시 **2번 설치 (A) 경로(torch==2.6.0+cu124)**로
설치되어 있어야 합니다.
...
- 모든 데이터/베이스 모델 경로는 `config.py` 기준 상대 경로로 관리됩니다(§3, §4 참고).

## 7. 프로젝트 구조 (버전 히스토리 v1-v21)
