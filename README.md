...
seed 고정, cuDNN deterministic, TF32 비활성화, 고정 배치 크기 등 가능한 결정론적 설정을 전부
적용했습니다. 다만 bf16 + 4bit 양자화 + attention 커널의 GPU 연산은 누적 순서가 완전히 고정되지
않아, 동일 설정으로 반복 실행해도 로짓의 하위 비트가 미세하게 달라질 수 있고 24-way 분류에서
1·2등 점수가 근소한 일부 샘플은 예측이 뒤집힐 수 있습니다. 이는 GPU 추론 전반에 알려진 현상이며,
학습 하드웨어(H100)와 재현 환경(RTX 3090)이 다르면 폭이 더 커질 수 있습니다. 동일 설정 재실행 시
2-4% 내외의 행이 달라질 수 있는 것으로 알려져 있고, 결과에 큰 차이가 없다면 정상적인 numerical
discrepancy입니다.
