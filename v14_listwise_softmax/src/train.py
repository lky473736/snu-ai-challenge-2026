"""
HNTV v14 Training — DDP via Accelerate
v14: v8 레시피(LoRA r=128/alpha=256, LR=5e-5, EPOCHS=5, hard_negative.py K=7 d=1..6 전 구간)를
100% 그대로 두고, loss만 AdaptiveDistanceLoss -> ListwiseSoftmaxLoss(8-way joint softmax,
distance 가중치 없음)로 교체한 단일 변수 실험. (src/loss.py 참고)
...
def _forward_chunk(model_unwrapped, processor, texts, imgs_list, yes_id, no_id, device, chunk_size):
    """OOM 나면 chunk_size를 절반으로 줄여 재시도"""
    while True:
        try:
            parts = []
...
def main():
    ...
    parser.add_argument("--ckpt_name",  type=str,   default="best_v14")
    ...
    if is_main:
        ...
        print(f"[v14] Processes={accelerator.num_processes}  Device={device}")
        print(f"  lora_r={args.lora_r}  lora_alpha={args.lora_alpha}  lr={args.lr}")
        print(f"  Loss: ListwiseSoftmaxLoss (8-way joint softmax, distance 가중치 없음, "
              f"v8과 동일 hard-negative 구성 K=7 d=1..6 전 구간)")
        print(f"  Hard negatives: LIVE resampled every __getitem__ (v8과 동일)")
        if args.resume_from:
            print(f"  [resume] {args.resume_from} 에서 재출발, start_epoch={args.start_epoch}")

    torch.manual_seed(SEED)

    # ── 데이터 (v8과 동일 SEED/VAL_RATIO → 동일 val split) ──────────
    if is_main:
    ...
    if is_main and not (args.resume_from and weight_log_path.exists()):
        with open(weight_log_path, "w", newline="") as f:
            ...
            # 주의(v14): w1~w6은 균등(1/6) placeholder, ema1~6은 미사용 placeholder(0).
            # L1~L6만 실제 의미 있는 진단값 — distance d negative가 평균적으로 받은 softmax 확률.
