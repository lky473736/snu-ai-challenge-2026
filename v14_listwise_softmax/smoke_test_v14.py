...
print(f"LORA_R={LORA_R}  LORA_ALPHA={LORA_ALPHA}  (alpha/r={LORA_ALPHA/LORA_R:.2f}, v8과 동일해야 함)")
...
loss, per_dist_diag, weights = criterion(torch.cat(ord_logits_parts), ord_dists, ord_sizes)
print("loss:", loss.item(), "finite:", torch.isfinite(loss).item())
print("per_dist avg_neg_prob (diag only):", per_dist_diag)
print("weights (uniform placeholder):", weights.tolist())

loss.backward()
print("backward 성공")

peak = torch.cuda.max_memory_allocated() / 1e9
print(f"Peak VRAM: {peak:.2f} GB  (v8 r=128 기준과 거의 동일해야 함 — loss만 바뀌었으므로)")
